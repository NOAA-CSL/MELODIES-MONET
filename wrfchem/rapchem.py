...
def open_mfdataset(fname,
    ...
    pm25 = getvar(wrflist,'PM2_5_DRY',timeidx=ALL_TIMES,method='cat',squeeze=False)
    ...
    dset = xr.merge([o3,no,no2,so2,pm25,pres,tk])
  
    #sum pm25 variables and nox
    #dset = add_lazy_pm25(dset)
    dset = add_lazy_nox(dset)
    
    #Assign same attributes as before. The units are the same, but different format ug m^-3
    #dset['pm25'] = dset['pm25'].assign_attrs({
    #        'units': '$\mu g m^{-3}$','name': 'PM2.5','long_name': 'PM2.5'})
    #dset = dset.rename({'pm25': 'PM25'})
