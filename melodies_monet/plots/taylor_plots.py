...
def make_taylor_diagram(df, col1, col2, scale, savename, date=None):
    dia = monet.plots.plots.taylordiagram(df, col1=col1, col2=col2, label1='AIRNOW', label2='CMAQ', scale=scale)
    date = df.time.min()
...
def make_plots(df, variable, obs_variable, startdate, enddate, scale, out_name):
    if startdate == None and enddate == None:
        for t in df.time.unique():
            ...
            print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
            odf = df.loc[df.time == date, ['time', 'latitude', 'longitude', obs_variable, variable]]
            if ~odf.empty:
